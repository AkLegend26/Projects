...
void init_test() {
    ...
    if (b.getNumRem() != 0) {
        std::cout << "Something is wrong with initializer" << std::endl;
    }
    b = a;
    if (b.getNumRem() != 3) {
        std::cout << "Assignment operator not working properly" << std::endl;
    }
    if (b.getRem(0) != rem3) {
        std::cout << "Rem wasn't copied properly." << std::endl;
    }
    if (b.getRem(1) != rem2) {
        std::cout << "Rem wasn't copied properly." << std::endl;
    }
...
int main() {
    ...
    try {
        ...
        std::cout << "No error was thrown when adding multiple reminders."<< std::endl;
    }
